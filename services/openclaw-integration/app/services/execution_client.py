...
DOMAIN_INSTRUCTIONS: dict[str, str] = {
    "web": (
        ...
        "The plan contains create_file operations with REAL production code — "
        "commit each file EXACTLY as provided in the operations list without modification. "
        "The code uses Next.js App Router with TypeScript and Tailwind CSS v4. "
        "For provision_repo: create a GitHub repository with the specified name and settings. "
        "For provision_hosting: create a Vercel project linked to the repository. "
        "For create_file/write_config: write the exact file content to the repo at the specified path. "
        "For build: run the specified command (smoke checks). "
        "For deploy: trigger deployment via Vercel. "
        "Preserve all file content exactly — do not reformat, simplify, or modify generated code. "
...
def _plan_to_openresponses_body(plan: dict, task_id: Optional[str] = None) -> dict:
    ...
    if plan.get("executor_contract") == "deterministic_web_v1" or isinstance(plan.get("execution_plan_v2"), dict):
        instructions += (
            ...
            "The operations contain production-ready code generated by the skills engine. "
            "Commit all create_file operations to the repository preserving exact content and paths. "
            "Run npm install after writing package.json, then execute build/smoke commands. "
            "Deploy the built site to Vercel and return the deployment URL."
        )
    return {
        "model": "openclaw:main",
        "user": user,
